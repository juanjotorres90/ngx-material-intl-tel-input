...
- Add native Angular 22 Signal Forms support through `FormValueControl`.
- Add standard Reactive Forms `formControlName` support while preserving the
  existing `fieldControl` and `fieldControlName` APIs.
- Export `telephoneNumberSchema()` for composable Signal Forms validation.
- Add standalone two-way `[(value)]` binding plus `focus()` and `reset()`.
...
- Add 'localizeCountryNames' option to localize country names based on the app locale ([#57](https://github.com/juanjotorres90/ngx-material-intl-tel-input/issues/57)).
