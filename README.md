...
Validation with [libphonenumber-js](https://gitlab.com/catamphetamine/libphonenumber-js) (Google's libphonenumber metadata, a fraction of the bundle size)
...
Ensure to include `provideAnimations()` in the providers array of your main.ts file to initialize your application with animations. If you enable `autoIpLookup`, also include `provideHttpClient()`. Refer to the Angular Docs [provideAnimations](https://angular.dev/api/platform-browser/animations/provideAnimations#) and [provideHttpClient](https://angular.dev/api/common/http/provideHttpClient#) for detailed instructions.

> **Privacy note:** `autoIpLookup` is disabled by default. When enabled, it sends a request to a third-party, rate-limited geolocation service ([ipapi.co](https://ipapi.co)) to detect the user's country from their IP. You can point it at your own endpoint (or change the timeout) by providing the `GEO_IP_CONFIG` token:
>
> ```typescript
> import { GEO_IP_CONFIG } from 'ngx-material-intl-tel-input';
>
> providers: [{ provide: GEO_IP_CONFIG, useValue: { url: 'https://my-api.example.com/geo', timeoutMs: 3000 } }];
> ```
...
The component implements `ControlValueAccessor`, so it integrates with Angular forms like any other form control:

```html
<form [formGroup]="formGroup">
  <ngx-material-intl-tel-input formControlName="phone" [required]="true"> </ngx-material-intl-tel-input>
</form>
```

It also works with `[formControl]` and `ngModel`. The legacy `[fieldControl]` / `fieldControlName` inputs remain supported but are deprecated:

```html
<form [formGroup]="formGroup">
  <ngx-material-intl-tel-input [fieldControl]="control" [required]="true"> </ngx-material-intl-tel-input>
</form>
```
...
| ------------------------ | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| fieldControl             | `FormControl \| AbstractControl \| null` | `FormControl('')`                                                                                                                                                                                                                                                                                                                                      | **Deprecated** — bind with `formControlName` / `[formControl]` / `ngModel` instead. Form control used to retrieve and set the value.                                                                                                                                                |
| fieldControlName         | `string`                                 | `''`                                                                                                                                                                                                                                                                                                                                                   | **Deprecated** — bind with `formControlName` instead. Form control name to assign the control from a FormGroup.                                                                                                                                                                     |
...
| autoIpLookup             | `boolean`                                | `false`                                                                                                                                                                                                                                                                                                                                                | Sets initial country code based on user's ip (sends the user's IP to a geolocation service; see the privacy note above).                                                                                                                                                            |
...
| defaultCountry           | `CountryISO \| string`                   | `CountryISO.Spain`                                                                                                                                                                                                                                                                                                                                     | Country selected when no better candidate is available (auto-select and geo-IP failed).                                                                                                                                                                                             |
...
| outputNumberFormat       | `PhoneNumberFormat`                      | `PhoneNumberFormat.INTERNATIONAL`                                                                                                                                                                                                                                                                                                                      | Sets the output number format to INTERNATIONAL, E164, or RFC3966 format. Import `PhoneNumberFormat` from `ngx-material-intl-tel-input`.                                                                                                                                             |
...
Numeric `PhoneNumberFormat` values from `google-libphonenumber` remain
supported for backwards compatibility, but are deprecated. New code should
import the string-valued `PhoneNumberFormat` enum from this package.
